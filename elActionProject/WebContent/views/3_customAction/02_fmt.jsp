...
<body>
	...
	그냥 출력 : ${ num1 } <br>
	세자리 마다 구문하여 출력 : <fmt:formatNumber value="${ num1 }"/> <!-- groupingUsed 기본값 (true) == 구분자 있다. --> <br>
	숫자 그대로 출력 : <fmt:formatNumber value="${ num1 }" groupingUsed="false"/>
	<!-- groupingUsed : 세자리마다 구분자(,) 표시 여부 지정 --> <br>
	...
	<ul>
		<li>현재 날짜 : <fmt:formatDate value="${ current }"/></li> <!-- type 생략시 기본값이 date -->
		<li>현재 시간 : <fmt:formatDate value="${ current }" type="time"/></li>
		<li>현재 날짜 및 시간 : <fmt:formatDate value="${ current }" type="both"/></li> <!-- dateStyle, timeStyle 생략시 기본값이 medium -->
